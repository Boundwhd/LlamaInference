...
__global__ void rope_kernel_cu_fp32(int pos, int dim, int kv_dim, int head_size,
    const float* input_q, const float* input_k, const float* sin_cache, const float* cos_cache) {
...
__global__ void rope_kernel_cu_fp32(int pos, int dim, int kv_dim, int head_size,  const float* input_q, const float* input_k,
    const float* sin_cache, const float* cos_cache) {
...
void sin_cos_cache_calc_cu(int head_size, int max_seq_len, const tensor::Tensor& sin_cache,
    ...
    if (stream) {
        sin_cos_calc<<<1, threads, 0, stream>>>(head_size, max_seq_len,
            const_cast<float*>(sin_cache.ptr<float>()), const_cast<float*>(cos_cache.ptr<float>()));
    } else {
